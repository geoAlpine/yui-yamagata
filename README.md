...
- **物資配布（`supply`）・災害ごみ仮置場（`waste`）にデータ源がない。**
  いずれもOSMに無く、自治体が災害時に発表するもの。
  利用者の報告と自治体からの登録に依存する。
- **アクセス集中への備え。** 熊本地震のイマココナビは7万人/日を捌いた。
  同規模が来た場合、単一VPSでは持たない。CDNの前段配置が必要。
- **バックアップの外部退避先。** 現在は同一VPS内。サーバごと失うと復旧できない。
