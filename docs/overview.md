...
後半5種類は、位置をあらかじめ登録していません。断水・停電と通行止めは「場所」ではなく状況そのものなので、住民が地点を追加して報告します。携帯・Wi-Fi も同様です。

**災害ごみ仮置場は、事前に登録できる余地があると考えています。** 発災後に白紙から決まるものではなく、市町村の災害廃棄物処理計画で候補地が選定されているためです（9章）。物資配布・炊き出しのみ、発災後に決まる真に動的な情報です。
...
- **投稿内容の事前審査は行いません。** 誤情報が一時的に掲載される可能性は残ります。4章の仕組みで事後に補正します
- **物資配布・炊き出しの情報源がありません。** 発災後に決まるため事前登録ができず、現状は住民の報告に依存します（9章でご相談させてください）
- **データベースの定期バックアップが未整備です。** サーバを失った場合、住民から寄せられた報告は復旧できません。参照データ（避難所・施設の位置）は外部データから再取得できます
- **投稿（書き込み）はCDNで肩代わりできません。** 閲覧は下記のとおり対処済みですが、報告の受け取りは必ずサーバまで届きます。極端な集中時にはここが先に詰まります。なお写真の縮小とEXIF除去は利用者の端末側で行うため、サーバ側で画像処理は発生しません

### 大規模アクセスについて

熊本地震時の同種サービスは7万人／日を処理しました。同規模の集中を1台のサーバで受けるのは無理であり、かつこのサーバには他の商用サイトが同居しているため、**巻き添えで停止させてしまうことが最大のリスク**です。

そのため、以下を実施しています。

- **閲覧はCDNで受けます。** 一覧・場所の詳細・お知らせ・位置情報による並べ替えのいずれも30秒キャッシュしており、**同じ画面へのアクセスがどれだけ増えても、サーバへの到達は30秒に1回程度**まで下がります（CDNの拠点ごと）
- **資源の上限を意図的に絞っています。** CPU・メモリ・データベース接続数に上限を設け、**同居する他サイトを巻き込む前に、このサイトが先に停止する**ようにしてあります

「無制限に耐える」ではなく「限界を超えても被害を自分の中に留める」という設計です。

#### 負荷試験の結果（2026年8月7日実施）

CDNを外し、サーバ本体の性能を実測しました。同時接続を5から80まで上げても**エラーは1件も発生しませんでした**。

| 経路 | 1秒あたりの処理数 | 応答時間（同時40接続） |
|---|---|---|
| 施設の検索（データのみ） | 約550件 | 76ミリ秒 |
| 場所の詳細ページ | 約180件 | 224ミリ秒 |
| 報告フォーム | 約115件 | 348ミリ秒 |

7万人／日の利用があり、そのうち2割が最も混む1時間に集中したと仮定すると、平均で毎秒4件程度です。瞬間的に10倍が来ても毎秒40件で、**上記の数値には余裕があります。** CDNが処理する分を除けばサーバに届く量はさらに減ります。

なお処理数の上限はCPUではなく、データベース接続数（意図的に8本に制限）によるものです。同居する他サイトの資源を奪わないための設定で、必要であれば引き上げる余地があります。

---

## 9. ご相談させていただきたいこと

このサイトが埋められない部分は、**県が既にお持ちの情報で埋まる**ものがあります。以下2点をご相談させてください。

### （1）災害廃棄物の仮置場の候補地データ

災害ごみの仮置場は、発災後に白紙から決まるものではなく、**市町村の災害廃棄物処理計画で候補地が事前に選定されている**と承知しています。

候補地の一覧をご提供いただければ、**平時のうちにサイトへ登録できます。** 指定避難所を国土地理院のデータから取り込んでいるのと同じ扱いになります。発災後は住民が「搬入できる／混雑・待ち時間あり／受付終了」を報告するだけで済みます。

床上浸水の片付けが始まる時期に、「どこへ持ち込めるのか」「いま並んでいるのか」が分からないことは、実際の負担として大きいと考えています。

### （2）L-Alert（災害情報共有システム）の受信について

物資配布・炊き出しは発災後に決まるため、事前登録ができません。自動で取得するには **L-Alert** が最も確実な経路と考えています。

受信するには情報伝達者としての登録が必要と承知しており、**県は情報発信者側**にあたります。受信の可否・条件についてご教示いただけますでしょうか。

L-Alert から受け取れる場合、副次的な効果のほうが大きいと考えています。**指定避難所の開設状況を公式情報として表示できる**ためです。現在この部分は住民の報告に依存していますが、公式情報が入れば「公式が発表した開設状況」と「住民が見た混雑状況」を併記でき、両方そろって初めて役に立つ形になります。

なお、35市町村のホームページを機械的に収集する方法は採りません。書式が統一されておらず、災害時には各自治体のサーバ自体が不安定になるため、最も必要な場面で動かなくなる可能性が高いためです。
